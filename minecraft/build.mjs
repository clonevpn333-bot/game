// Bundle Three.js (base64 blob) + game.js (readable) + CSS into one index.html.
import fs from 'fs';
...
<body>
<canvas id="gl"></canvas>
<div id="boot">Loading BLOCKCRAFT…</div>
...
(function () {
  ...
  try {
    var threeURL = URL.createObjectURL(new Blob([decode(document.getElementById('three-src').textContent)],{type:'text/javascript'}));
    var game = document.getElementById('game-src').textContent.replace(/<\\\/script>/gi,'</scr'+'ipt>').replace('__THREE_URL__', threeURL);
    var gameURL = URL.createObjectURL(new Blob([game],{type:'text/javascript'}));
    import(gameURL).catch(fail);
  } catch(e){ fail(e); }
})();
</script>
</body>
