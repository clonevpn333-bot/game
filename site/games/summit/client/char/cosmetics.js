...
export const OUTFITS = {
  expedition: { name: 'Expedition Red', jacket: 0xb8402f, trousers: 0x2d3138, accent: 0xe8c07d, price: 0 },
  glacier:    { name: 'Glacier Blue',   jacket: 0x2f6fb8, trousers: 0x22262c, accent: 0xd8e6f2, price: 400 },
  forest:     { name: 'Forest Green',   jacket: 0x3d6b45, trousers: 0x35301f, accent: 0xc9b483, price: 400 },
  dust:       { name: 'Dust Tan',       jacket: 0xb09055, trousers: 0x4a4038, accent: 0x2f2a25, price: 650 },
  ash:        { name: 'Ashfall Black',  jacket: 0x2a2c30, trousers: 0x1d1f22, accent: 0xff7a3c, price: 900 },
  summit:     { name: 'Summit Gold',    jacket: 0xd8a13c, trousers: 0x2a2622, accent: 0xfff2cf, price: 1600 },
};
...
export function makeExtras(look, rest) {
  ...
  switch (look.hat) {
    case 'beanie': {
      const g = new THREE.SphereGeometry(0.122, 16, 10, 0, Math.PI * 2, 0, Math.PI * 0.58);
      g.scale(1, 1.06, 1.02);
      g.translate(head.x, head.y + 0.072, head.z);
      const brim = new THREE.TorusGeometry(0.116, 0.021, 8, 18);
      brim.rotateX(Math.PI / 2);
      brim.translate(head.x, head.y + 0.055, head.z);
      out.push({ geo: mergeGeometries([g, brim]), mat: 'gear' });
      break;
    }
    case 'helmet': {
      const g = new THREE.SphereGeometry(0.128, 18, 12, 0, Math.PI * 2, 0, Math.PI * 0.55);
      g.scale(1, 0.95, 1.05);
      g.translate(head.x, head.y + 0.078, head.z);
      const vent = new THREE.BoxGeometry(0.03, 0.02, 0.16);
      vent.translate(head.x, head.y + 0.19, head.z);
      out.push({ geo: mergeGeometries([g, vent]), mat: 'gear' });
      break;
    }
    case 'brim': {
      const cap = new THREE.SphereGeometry(0.118, 16, 10, 0, Math.PI * 2, 0, Math.PI * 0.5);
      cap.translate(head.x, head.y + 0.072, head.z);
      const brim = new THREE.CylinderGeometry(0.235, 0.235, 0.012, 22);
      brim.translate(head.x, head.y + 0.07, head.z);
      out.push({ geo: mergeGeometries([cap, brim]), mat: 'gear' });
      break;
    }
    case 'hood': {
      const g = new THREE.SphereGeometry(0.155, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.62);
      g.scale(1.02, 1.1, 1.16);
      g.translate(head.x, head.y + 0.045, head.z - 0.022);
      const neck = new THREE.CylinderGeometry(0.15, 0.19, 0.14, 16, 1, true);
      neck.translate(head.x, head.y - 0.06, head.z - 0.02);
      out.push({ geo: mergeGeometries([g, neck]), mat: 'jacket' });
      break;
    }
    case 'crown': {
      const band = new THREE.TorusGeometry(0.118, 0.016, 8, 20);
      band.rotateX(Math.PI / 2);
      band.translate(head.x, head.y + 0.10, head.z);
      const spikes = [];
      for (let i = 0; i < 7; i++) {
        const a = (i / 7) * Math.PI * 2;
        const s = new THREE.ConeGeometry(0.017, 0.075 + (i % 2) * 0.035, 5);
        s.translate(head.x + Math.cos(a) * 0.113, head.y + 0.15, head.z + Math.sin(a) * 0.113);
        spikes.push(s);
      }
