...
function updateEntity(game, e, dt) {
  ...
  /* apply movement */
  var accel = speed * (e.inWater ? 0.5 : 1) * 6;
  if (def.fly || (def.water && e.inWater)) {
    ...
    e.vx = clamp(e.vx, -speed * 22, speed * 22);
    e.vy = clamp(e.vy, -speed * 16, speed * 16);
    e.vz = clamp(e.vz, -speed * 22, speed * 22);
  } else {
    e.vx += mvX * accel * dt * 8;
    e.vz += mvZ * accel * dt * 8;
    var mx = speed * 9;
    ...
    if ((mvX || mvZ) && e.onGround && e.jumpCool <= 0) {
      var ax = Math.round(e.x + mvX * (e.w * 0.5 + 0.35));
      var az = Math.round(e.z + mvZ * (e.w * 0.5 + 0.35));
  ...
  e.walkAmt = clamp(hspd / Math.max(0.5, speed * 9), 0, 1) * (e.onGround || def.fly || def.water ? 1 : 0.35);
...
function applyPhysics(game, e, dt, def) {
  ...
  var before = e.vy;
  var r = collideAxis(world, e.dim, e, e.vx * dt, e.vy * dt, e.vz * dt);
  ...
  if (e.onGround) {
    ...
    e.vy = 0;
    var fr = Math.pow(0.02, dt);
    e.vx *= fr; e.vz *= fr;
  } else if (!def.fly && !(def.water && e.inWater)) {
    var af = Math.pow(0.45, dt);
    e.vx *= af; e.vz *= af;
  }
  if (Math.abs(r.dx - e.vx * dt) > 1e-7) e.vx = 0;
  if (Math.abs(r.dz - e.vz * dt) > 1e-7) e.vz = 0;
