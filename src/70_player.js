...
function rayBoxes(ox, oy, oz, dx, dy, dz, bx, by, bz, boxes, maxDist) {
  ...
  for (var i = 0; i < boxes.length; i++) {
    ...
    /* The face we hit is the one we enter through: travelling in +X we cross
       the box's -X face, and vice versa — so the pairs are the negative face
       first, positive second. */
    var axes = [[ox, dx, x0, x1, 1, 0], [oy, dy, y0, y1, 3, 2], [oz, dz, z0, z1, 5, 4]];
    var ok = true;
    for (var a = 0; a < 3; a++) {
