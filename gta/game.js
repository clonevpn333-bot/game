...
function toonRamp(steps) {
  const d = new Uint8Array(steps); for (let i = 0; i < steps; i++) d[i] = Math.round(40 + 215 * (i / (steps - 1)));
...
class City {
  ...
  _tower(x, z, w, d, h, color, rng, G) {
    const { map, emissive } = facadeTextures(w, h, '#' + color.toString(16).padStart(6, '0'));
    const side = () => mat(0xffffff, { map: map.clone(), emissiveMap: emissive.clone(), emissive: 0xffffff, emissiveIntensity: 1.05 });
    const top = mat(color);
    const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), [side(), side(), top, top, side(), side()]); m.position.set(x, h / 2, z); m.castShadow = true; m.receiveShadow = true; G.add(m);
    // structural trim: base, roof parapet, mid cornices (reads as real stacked floors)
    const trimMat = mat(new THREE.Color(color).multiplyScalar(0.8).getHex());
    const base = new THREE.Mesh(new THREE.BoxGeometry(w + 0.8, 1.4, d + 0.8), trimMat); base.position.set(x, 0.7, z); base.castShadow = true; G.add(base);
  ...
  onRoad(x, z) { const half = this.size / 2, lx = ((x + half) % this.cell + this.cell) % this.cell, lz = ((z + half) % this.cell + this.cell) % this.cell; return lx < this.road || lz < this.road; }
  snapSidewalk(x, z) {
...
const HAIR = [0x241810, 0x0e0e0e, 0x6b4a2a, 0xd0a63a, 0x7a3a20, 0x4a4a4a, 0x2b2b2b];
function cap(r, len, m, rs) { return new THREE.Mesh(new THREE.CapsuleGeometry(r, len, 8, rs || 16), m); }
function humanFigure(p) {
  const skin = p.skin || pick(SKIN), shirt = p.shirt || pick(SHIRTS), pants = p.pants || pick(PANTS), hair = p.hair || pick(HAIR), longSleeve = Math.random() < 0.5;
  ...
  const pelvis = new THREE.Mesh(new THREE.SphereGeometry(0.18, 24, 18), Pa); pelvis.scale.set(1.05, 0.72, 0.8); hips.add(pelvis);
  const chest = new THREE.Group(); chest.position.y = 0.12; hips.add(chest);
  const prof = [[0.03, 0], [0.17, 0.03], [0.2, 0.16], [0.225, 0.3], [0.235, 0.42], [0.185, 0.5], [0.09, 0.54], [0.03, 0.55]].map(a => new THREE.Vector2(a[0], a[1]));
  const torso = new THREE.Mesh(new THREE.LatheGeometry(prof, 32), Sh); torso.scale.z = 0.68; chest.add(torso);
  const head = new THREE.Group(); head.position.y = 0.56; chest.add(head);
  head.add(new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.08, 0.11, 14), Sk)).position.y = -0.03;
  const skull = new THREE.Mesh(new THREE.SphereGeometry(0.168, 32, 26), Sk); skull.position.y = 0.12; skull.scale.set(0.95, 1.05, 1.0); head.add(skull);
  const jaw = new THREE.Mesh(new THREE.SphereGeometry(0.128, 24, 18), Sk); jaw.position.set(0, 0.04, 0.02); jaw.scale.set(0.88, 0.76, 0.94); head.add(jaw);
  for (const sx of [-1, 1]) {
    const eye = new THREE.Mesh(new THREE.SphereGeometry(0.034, 14, 12), mat(0xffffff)); eye.position.set(sx * 0.058, 0.13, 0.132); head.add(eye);
    const iris = new THREE.Mesh(new THREE.SphereGeometry(0.017, 12, 10), mat(0x241a12)); iris.position.set(sx * 0.058, 0.13, 0.156); head.add(iris);
    const brow = new THREE.Mesh(new THREE.BoxGeometry(0.064, 0.015, 0.02), Ha); brow.position.set(sx * 0.058, 0.175, 0.138); head.add(brow);
    const ear = new THREE.Mesh(new THREE.SphereGeometry(0.036, 12, 10), Sk); ear.position.set(sx * 0.158, 0.11, 0.01); ear.scale.set(0.5, 1, 0.7); head.add(ear);
  }
  const nose = new THREE.Mesh(new THREE.ConeGeometry(0.027, 0.075, 10), Sk); nose.position.set(0, 0.1, 0.158); nose.rotation.x = Math.PI / 2 + 0.4; head.add(nose);
  head.add(new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.015, 0.016), mat(0x8a4444))).position.set(0, 0.05, 0.148);
  const hairCap = new THREE.Mesh(new THREE.SphereGeometry(0.177, 28, 22, 0, TAU, 0, 1.72), Ha); hairCap.position.set(0, 0.13, -0.006); hairCap.scale.set(1, 1.03, 1.04); head.add(hairCap);
  const arms = {};
  for (const s of ['L', 'R']) {
    const sx = s === 'L' ? -1 : 1; const arm = new THREE.Group(); arm.position.set(sx * 0.22, 0.44, 0); arm.rotation.z = sx * 0.08; chest.add(arm);
    const up = cap(0.058, 0.24, longSleeve ? Sh : Sk); up.position.y = -0.15; arm.add(up);
    const fore = new THREE.Group(); fore.position.y = -0.3; arm.add(fore);
    fore.add(cap(0.05, 0.22, longSleeve ? Sh : Sk)).position.y = -0.14;
    const hand = new THREE.Mesh(new THREE.SphereGeometry(0.066, 14, 12), Sk); hand.position.y = -0.3; hand.scale.set(1, 1.3, 0.55); fore.add(hand);
    arms[s] = { arm, fore };
  }
  const legs = {};
  for (const s of ['L', 'R']) {
    const sx = s === 'L' ? -1 : 1; const leg = new THREE.Group(); leg.position.set(sx * 0.1, 0, 0); hips.add(leg);
    leg.add(cap(0.11, 0.32, Pa)).position.y = -0.2;
    const shin = new THREE.Group(); shin.position.y = -0.42; leg.add(shin);
    shin.add(cap(0.086, 0.32, Pa)).position.y = -0.2;
    const foot = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.1, 0.3), Bo); foot.position.set(0, -0.45, 0.08); shin.add(foot);
    legs[s] = { leg, shin };
  }
...
function animateHuman(f, dt, o) {
  ...
  if (state === 'walk' || state === 'run') {
    ...
    j.armL.rotation.x = b * 0.05; j.armR.rotation.x = -b * 0.05; j.armL.rotation.z = 0.1; j.armR.rotation.z = -0.1; j.foreL.rotation.x = -0.2; j.foreR.rotation.x = -0.2;
...
class Car {
  ...
  update(dt) {
    ...
    if (!this.ai) return;
    const p = this.game.player; if (this.pos.distanceTo(p.pos) > 175) { this.game.scene.remove(this.root); this.game.cars = this.game.cars.filter(c => c !== this); this.game.spawnTraffic(); return; }
    this.turnCd -= dt; this.speed = lerp(this.speed, 12, dt * 2);
    const fx = Math.sin(this.yaw), fz = Math.cos(this.yaw), ahead = this.game.city.onRoad(this.pos.x + fx * 5, this.pos.z + fz * 5);
    if (!ahead || (this.turnCd <= 0 && Math.random() < 0.4)) { const opts = [this.yaw, this.yaw + Math.PI / 2, this.yaw - Math.PI / 2].filter(y => this.game.city.onRoad(this.pos.x + Math.sin(y) * 6, this.pos.z + Math.cos(y) * 6)); this.yaw = opts.length ? pick(opts) : this.yaw + Math.PI; this.turnCd = 1.5 + Math.random(); }
    this._move(dt);
  }
