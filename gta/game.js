...
class Car {
  ...
  damageCar(amt, at) {
    ...
    this.maxHp = this.maxHp || 120; this.hp = (this.hp == null ? this.maxHp : this.hp) - amt;
    if (amt > 5) this._dent();
    if (this.hp < this.maxHp * 0.42 && !this.smoking) this.smoking = true;
    if (this.hp < this.maxHp * 0.16 && !this.burning) this.burning = true;
    if (this.hp <= 0 && !this._boom) { this.hp = 0; this._boom = true; this.game.explode(this.pos.clone().setY(1.0), 1.2, this.color); this.game.leaveWreck(this); this.removeMe = true; if (this.driver === this.game.player) { this.game.player.inCar = null; this.game.player.root.visible = true; this.game.camShake = 1.5; } }
  }
  _dent() {
    const kids = this.root.children.filter(c => c.isMesh && c.geometry && c.geometry.type === 'BoxGeometry' && c !== this._spill && c !== this._rear);
    if (!kids.length) return; const m = kids[(Math.random() * kids.length) | 0];
    m.scale.x *= 0.92 + Math.random() * 0.05; m.scale.y *= 0.9 + Math.random() * 0.05; m.position.x += rnd(-0.05, 0.05); m.position.y -= rnd(0, 0.04); m.rotation.z += rnd(-0.06, 0.06);
  }
  updateDamage(dt) {
