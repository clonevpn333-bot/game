...
class City {
  ...
  build() {
    ...
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(span + 800, span + 800), mat(0x6a6c76, { map: noiseTexture(70, 72, 82, 14) }));
    ...
    // ---- the ocean east of Vice Beach ----
    const water = new THREE.Mesh(new THREE.PlaneGeometry(400, span + 800), mat(0x2f83c6, { emissive: 0x1d4e7a, emissiveIntensity: 0.25, transparent: true, opacity: 0.9 }));
    water.rotation.x = -Math.PI / 2; water.position.set(half + 260, 0.06, 0); this.group.add(water); this.water = water;
    const sand = new THREE.Mesh(new THREE.PlaneGeometry(150, span + 800), mat(0xd6c493)); sand.rotation.x = -Math.PI / 2; sand.position.set(half + 70, 0.03, 0); this.group.add(sand);
    ...
    this.places = { tonyDiner: LC(1, 4), salGarage: LC(1, 1), dezzyClub: LC(6, 4), docks: LC(3, 0), strip: LC(7, 6), waterfront: { x: (this.bayX0 + this.bayX1) / 2, z: -half + 6 * CELL }, finale: { x: (this.bayX0 + this.bayX1) / 2, z: half } };
...
class Game {
  ...
  start() {
    ...
    if (this.saveData) { this.player.money = this.saveData.money != null ? this.saveData.money : this.player.money; this.player.hasGun = !!this.saveData.gun; if (this.player.hasGun) this.player.weapon = 'pistol'; }
  }
  spawnPed(cluster) {
  ...
  updateBusted(dt) {
    const p = this.player;
    if (p.wanted > 0 && !p.inCar) {
      let near = false; for (const n of this.npcs) if (n.cop && !n.dead && n.pos.distanceTo(p.pos) < 1.9) { near = true; break; }
      this._bustT = near ? (this._bustT || 0) + dt : 0;
      if (this._bustT > 1.1 && !p.dead) { p.dead = true; this.saveGame(); this.ui.bustedOrDead('BUSTED'); }
    } else this._bustT = 0;
  }
  saveGame() { try { localStorage.setItem('nb_save', JSON.stringify({ ch: Math.max(0, this.story.mi), money: this.player.money | 0, gun: !!this.player.hasGun })); } catch (e) {} }
  ...
  loop() {
    ...
    if (this.menuMode) { this.menuCam(); this._ambient(dt); }
      ...
      else if (!frozen) { this.player.update(dt); for (const n of this.npcs) n.update(dt); for (const c of this.cars) c.update(dt); this.story.update(dt); this.updateWanted(dt); this.updatePickups(dt); this.updateBusted(dt); this.cull(); this.player.updateCamera(dt, false); }
...
class Player {
  ...
  update(dt) {
    ...
    if (this.gunCd <= 0) {
      ...
      else if (!armed && inp.mLe) { this.gunCd = 0.42; this.punchT = 0.3; this._punch(); }
    }
  ...
  _stats(dt) { this.regen += dt; if (this.health < this.maxHealth && this.wanted === 0 && this.regen > 2) this.health = Math.min(this.maxHealth, this.health + 6 * dt); if (this.health <= 0 && !this.dead) { this.dead = true; this.game.saveGame(); this.game.ui.bustedOrDead('WASTED'); } }
...
class Ped {
  constructor(game, x, z, cop, opts) {
    opts = opts || {}; this.game = game; this.cop = !!cop; this.dance = !!opts.dance; this.loiter = !!opts.loiter; this.vendor = !!opts.vendor; this.homeless = !!opts.homeless; this.pos = new THREE.Vector3(x, 0, z); this.yaw = opts.yaw != null ? opts.yaw : rnd(TAU);
...
class Story {
  ...
  finish() { const m = MISSIONS[this.mi]; this.play(m.after, this.giver, () => { this.game.player.money += m.reward || 0; if (m.gun) { this.game.player.hasGun = true; this.game.player.weapon = 'pistol'; this.game.ui.toast('★ Pistol acquired — Left-click to fire, 1/2 to switch'); } this.game.ui.bigCard('MISSION PASSED', (m.reward ? '+$' + m.reward : '')); this.marker.visible = false; setTimeout(() => this.next(), 2600); }); }
