...
class City {
  ...
  build() {
    ...
    for (let ix = 0; ix < N; ix++) for (let iz = 0; iz < N; iz++) {
      ...
      const r = rng();
      if (r < 0.05) this._camp(cx, cz, LOT, rng, G);      // homeless encampment
      else if (r < 0.15) this._park(cx, cz, LOT, rng, G);
    ...
    this._airport(half);
    // hidden packages (collectibles)
    this.packages = [];
    for (let i = 0; i < 12; i++) {
      const [px2, pz2] = this.snapSidewalk(rnd(-half + 14, half - 14), rnd(-half + 14, half - 14));
...
class Player {
  ...
  update(dt) {
    ...
    const inWater = this.pos.z > this.game.city.size / 2 + 95;
    ...
    // swimming in the bay: slow bobbing wade, no jumping
    const swim = this.pos.z > this.game.city.size / 2 + 95;
