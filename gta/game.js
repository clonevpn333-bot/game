...
function mat(c, o) {
  ...
  for (const k of ['emissive', 'emissiveIntensity', 'map', 'emissiveMap', 'transparent', 'opacity', 'side', 'depthWrite', 'alphaTest', 'polygonOffset', 'polygonOffsetFactor', 'polygonOffsetUnits']) if (o[k] !== undefined) p[k] = o[k];
...
class City {
  ...
  build() {
    ...
    const roadMat = mat(0x4a4c58, { map: noiseTexture(46, 48, 58, 10), emissive: 0x0a0b13, emissiveIntensity: 1 }); roadMat.map.repeat.set(2, 46); this.roadMat = roadMat; // kept so night can wet-shine the asphalt
...
class Game {
  constructor() {
    ...
    this.rain = makeRain(4200); this.scene.add(this.rain.pts); this.rain.pts.visible = false; this.rainAmt = 0; this.camShake = 0;
  ...
  shootRay(origin, dir, dmg, range) {
    ...
    else if (hitCar) { hitCar.hp = (hitCar.hp == null ? 120 : hitCar.hp) - dmg * 1.4; this.hitFx(end, 0xffd27a, 8); if (hitCar.hp <= 0 && !hitCar._boom) { hitCar._boom = true; this.explode(hitCar.pos.clone().setY(1.0), 1.15); hitCar.removeMe = true; if (hitCar.driver === this.player) { this.player.inCar = null; this.player.root.visible = true; } } }
  ...
  updateDayNight() {
    ...
    this.hemi.intensity = 0.26 + day * 0.92 + dusk * 0.4; this.scene.fog.color.copy(mixc(0xcfe0ee, 0x5a3a5e, 0x0a0a14));
    ...
    if (this.post && this.post.enabled) { this.post.compMat.uniforms.strength.value = 0.72 + night * 1.05; this.post.brightMat.uniforms.threshold.value = 1.0 - night * 0.62; this.post.compMat.uniforms.exposure.value = 1.4 - night * 0.16; }
    if (this.city.roadMat) this.city.roadMat.emissiveIntensity = 1 + night * 2.6; // rain-slicked reflectivity
    // clear days, rain rolling in at dusk and pouring after dark; the ground glow pools neon under you
    this.rainAmt = clamp(night * 0.95 + dusk * 0.22, 0, 1); if (this.rain) this.rain.pts.visible = this.playing && this.rainAmt > 0.05;
    if (this.rain) this.rain.mat.opacity = 0.3 + night * 0.4;
  ...
  loop() {
    ...
    if (this.menuMode) { this.menuCam(); this._ambient(dt); }
      ...
      this.updateFx(dt); this.updateRain(dt); this.ui.update(); this.updateCulling();
    }
    ...
    this.updateDayNight();
    if (this.playing) this.updateTraffic(dt);
  ...
  updateFx(dt) {
    ...
    for (const f of this.fx) {
      ...
      if (f.explode) { // fireball: expands + color-lerps white-hot → yellow → orange → ash, then fades
        ...
        if (t < 0.14) col.setRGB(1, 1, 0.96); else if (t < 0.34) col.setRGB(1, 0.86, 0.34); else if (t < 0.62) col.setRGB(1, 0.44, 0.12); else col.setRGB(0.42, 0.42, 0.45);
    ...
    this.fx = this.fx.filter(f => { if (f.life <= 0) { if (f.flash) { this.scene.remove(f.flash); } else { this.scene.remove(f.m); f.g.dispose(); f.m.material.dispose(); } return false; } return true; });
    this.camShake = Math.max(0, (this.camShake || 0) - dt * 2.4);
  }
  ...
  // ---- explosion: a fireball particle burst + blast light + proximity screen-shake ----
  explode(pos, scale) {
    scale = scale || 1; const n = 64, g = new THREE.BufferGeometry(), ps = new Float32Array(n * 3), vs = new Float32Array(n * 3);
    for (let i = 0; i < n; i++) { ps[i * 3] = pos.x; ps[i * 3 + 1] = pos.y; ps[i * 3 + 2] = pos.z; const a = Math.random() * TAU, e = Math.acos(rnd(-1, 1)), sp = rnd(7, 22) * scale; vs[i * 3] = Math.sin(e) * Math.cos(a) * sp; vs[i * 3 + 1] = Math.abs(Math.cos(e)) * sp * 0.7 + 5; vs[i * 3 + 2] = Math.sin(e) * Math.sin(a) * sp; }
    g.setAttribute('position', new THREE.BufferAttribute(ps, 3));
    const baseSize = 1.7 * scale, m = new THREE.Points(g, new THREE.PointsMaterial({ color: 0xffffff, size: baseSize, transparent: true, opacity: 1, depthWrite: false, blending: THREE.AdditiveBlending }));
    this.scene.add(m); this.fx.push({ m, g, ps, vs, n, life: 1.15, dur: 1.1, explode: true, baseSize, age: 0 });
    const fl = new THREE.PointLight(0xffb060, 9 * scale, 46 * scale, 2); fl.position.set(pos.x, pos.y + 1, pos.z); this.scene.add(fl); this.fx.push({ flash: fl, life: 0.42, age: 0 });
    const d = this.player.pos.distanceTo(pos); this.camShake = Math.max(this.camShake || 0, clamp(1 - d / 46, 0, 1) * 1.3 * scale);
    this.hitFx(pos, 0xff8a2a, 18);
    // catch anyone standing in the blast
    for (const nn of this.npcs) if (!nn.dead && nn.pos.distanceTo(pos) < 6 * scale) nn.damage(80);
...
class Car {
  constructor(game, x, z, dir, color, aiTraffic, kind) {
    this.game = game; this.pos = new THREE.Vector3(x, 0, z); this.speed = 0; this.driver = null; this.ai = true; this.aiTraffic = !!aiTraffic; this.turnCd = 1.5; this.kind = kind || null; this.color = color; this.hp = 120;
  ...
  control(dt, inp) {
    ...
    for (const o of this.game.cars) {
      ...
      if (dd < 3.6 && dd > 0.01) {
        ...
        if (hit > 6 && (this._ramCd || 0) <= 0) { this._ramCd = 0.4; this.game.hitFx(o.pos.clone().setY(1), 0xffe27a, 6); if (o.fleeing) { o._rams = (o._rams || 0) + 1; if (o._rams >= 2 && !o._boom) { o._boom = true; o.fleeing = false; o.ai = false; o.speed = 0; o.cruise = 0; this.game.explode(o.pos.clone().setY(1.0), 1.15); o.removeMe = true; } } }
