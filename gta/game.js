...
class Game {
  ...
  saveGame() { try { localStorage.setItem('nb_save', JSON.stringify({ ch: Math.max(0, this.story.mi), money: this.player.money | 0, gun: !!this.player.hasGun, own: this.city.shops.map((s, i) => s.owned ? i : -1).filter(i => i >= 0) })); } catch (e) {} }
  ...
  loop() {
    ...
    if (this.menuMode) { this.menuCam(); this._ambient(dt); }
      ...
      else if (!frozen) { this.player.update(dt); for (const n of this.npcs) n.update(dt); for (const c of this.cars) c.update(dt); this.story.update(dt); this.updateWanted(dt); this.updatePickups(dt); this.updateBusted(dt); this.updateIncome(dt); this.cull(); this.player.updateCamera(dt, false); }
