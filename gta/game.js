...
class City {
  ...
  build() {
    ...
    const sandM = mat(0xd6c493);
    const beachE = new THREE.Mesh(new THREE.PlaneGeometry(130, span + 240), sandM); beachE.rotation.x = -Math.PI / 2; beachE.position.set(half + 62, 0.03, 0); this.group.add(beachE);
    const beachW = new THREE.Mesh(new THREE.PlaneGeometry(70, span + 240), sandM); beachW.rotation.x = -Math.PI / 2; beachW.position.set(-half - 32, 0.03, 0); this.group.add(beachW);
    const beachN = new THREE.Mesh(new THREE.PlaneGeometry(span + 240, 70), sandM); beachN.rotation.x = -Math.PI / 2; beachN.position.set(0, 0.03, half + 32); this.group.add(beachN);
    const beachS = new THREE.Mesh(new THREE.PlaneGeometry(span + 240, 70), sandM); beachS.rotation.x = -Math.PI / 2; beachS.position.set(0, 0.03, -half - 32); this.group.add(beachS);
    const apron = new THREE.Mesh(new THREE.PlaneGeometry(340, 420), sandM); apron.rotation.x = -Math.PI / 2; apron.position.set(60, 0.02, -half - 150); this.group.add(apron); // airport peninsula
    ...
    for (let i = 0; i < 9; i++) {
      const bx2 = half + 26 + rnd(0, 30), bz2 = rnd(-half + 24, half - 24), uc = pick([0xff5fb0, 0x2fe6ff, 0xffd23a, 0x4dff9e]);
      const pole2 = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 2.4, 6), mat(0xe8e2d2)); pole2.position.set(bx2, 1.2, bz2); this.group.add(pole2);
      const top2 = new THREE.Mesh(new THREE.ConeGeometry(1.7, 0.85, 10), mat(uc)); top2.position.set(bx2, 2.5, bz2); top2.castShadow = true; this.group.add(top2);
      const tw = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.05, 2.0), mat(pick([0xff8a5a, 0x4fd0ff, 0xffe14a, 0xff5fb0]))); tw.position.set(bx2 + 1.7, 0.06, bz2 + rnd(-0.6, 0.6)); tw.rotation.y = rnd(-0.4, 0.4); this.group.add(tw);
    ...
    this.places = { tonyDiner: LC(1, 4), salGarage: LC(1, 1), dezzyClub: LC(6, 4), docks: LC(3, 0), strip: LC(7, 6), waterfront: { x: (this.bayX0 + this.bayX1) / 2, z: -half + 6 * CELL }, finale: { x: (this.bayX0 + this.bayX1) / 2, z: half }, gunshop: LC(4, 2), bank: LC(3, 4), home: this.homePos || LC(1, 6), motormax: LC(2, 2), jail: LC(0, 3), beach: { x: half + 42, z: 40 } };
  ...
  _jail(cx, cz, LOT, G) {
    ...
    const bw = 26, bd = 13, bh = 4.6, bz = cz - hw + 8, blkM = mat(0x848a94);
    const bf = new THREE.Mesh(new THREE.BoxGeometry(bw, 0.2, bd), mat(0x565b66)); bf.position.set(cx, 0.14, bz); bf.receiveShadow = true; G.add(bf);
    ...
    door.position.set(doorX, 0, frontZ); G.add(door); door.visible = false;
  ...
  jailDoorClose() { if (!this.jailDoor) return; this.jailOpen = false; this.jailDoor.visible = true; if (this.boxes.indexOf(this.jailDoorBox) < 0) this.boxes.push(this.jailDoorBox); }
  jailDoorOpen() { if (!this.jailDoor) return; this.jailOpen = true; this.jailDoor.visible = false; const i = this.boxes.indexOf(this.jailDoorBox); if (i >= 0) this.boxes.splice(i, 1); }
  // ---- AMMU-BAY: the island's legal-carry gun counter (walk in, E at the counter to buy) ----
  _gunshop(cx, cz, LOT, G) {
    const w = LOT - 12, d = LOT - 18, h = 5.8, t = 0.4, col = 0x4a5260, wallMat = mat(col);
    const floor = new THREE.Mesh(new THREE.BoxGeometry(w, 0.2, d), mat(0x2e333c)); floor.position.set(cx, 0.14, cz); floor.receiveShadow = true; G.add(floor);
  ...
  _bank(cx, cz, LOT, G) {
    const w = LOT - 10, d = LOT - 14, h = 9, t = 0.5, col = 0xd9d3c6, wallMat = mat(col);
    const floor = new THREE.Mesh(new THREE.BoxGeometry(w, 0.22, d), mat(0xbfb9ac)); floor.position.set(cx, 0.15, cz); floor.receiveShadow = true; G.add(floor);
  ...
  _houses(cx, cz, LOT, rng, G, mine) {
    ...
    const house = (hx, hz, open) => {
      ...
      if (open) { // your place: front door's always open for you
        const t2 = 0.4;
        const floor2 = new THREE.Mesh(new THREE.BoxGeometry(w2, 0.2, d2), mat(0x7a5a3c)); floor2.position.set(hx, 0.14, hz); floor2.receiveShadow = true; G.add(floor2);
        const wall2 = (ww, dd, ox, oz) => { const m = new THREE.Mesh(new THREE.BoxGeometry(ww, h2, dd), wallMat); m.position.set(hx + ox, h2 / 2, hz + oz); m.castShadow = true; m.receiveShadow = true; G.add(m); this.boxes.push({ x: hx + ox, z: hz + oz, hw: ww / 2 + 0.25, hd: dd / 2 + 0.25 }); };
  ...
  _venue(cx, cz, LOT, type, rng, G) {
    ...
    const wallCol = pick(FACADES), wallMat = mat(wallCol), floorMat = mat(type === 'club' ? 0x1a1428 : 0x33303d);
...
class Game {
  ...
  interact() {
    ...
    for (const rec of this.city.shops) {
      ...
      if (rec.type === 'bank') {
        ...
        if (rec.robT && this.time < rec.robT) { this.ui.toast('The vault is sealed tight — give it time'); return; }
        rec.robT = this.time + 600; const take = 600 + (Math.random() * 500 | 0); p.money += take; p.gunOutT = 6; this.addWanted(4);
  ...
  updateArrest(dt) {
    const p = this.player; if (p.dead || this.jailed) { this._bustT = 0; return; }
    ...
    if (p.wanted > 0 && !p.inCar && p.tazedT <= 0) {
      ...
      if (this._bustT > 1.0) this.goToJail('Restrained and cuffed');
    } else this._bustT = 0;
  }
  goToJail(why) {
    const p = this.player; if (this.jailed) return;
    this.jailed = true; p.tazedT = 0; p.wanted = 0; p.heat = 0; p.health = p.maxHealth;
...
class Player {
  constructor(game) {
    ...
    this.inCar = null; this.regen = 0; this.dead = false; this.hasGun = false; this.weapon = 'fists'; this.gunCd = 0; this.punchT = 0; this.shootT = 0; this.jackT = 0; this.jackCar = null; this.tazedT = 0; this.gunOutT = 0;
  ...
  startJack(car) {
    ...
    if (car.aiTraffic && !car.jacked) {
      car.jacked = true;
      // the yanked driver: dumped on the asphalt right at the door, out cold, then furious
      const owner = new Ped(this.game, car.pos.x + dx * 1.9, car.pos.z + dz * 1.9); owner.story = true; owner.knockT = 3.2; owner.angryT = 4; owner.yaw = this.yaw + Math.PI; this.game.npcs.push(owner);
  ...
  update(dt) {
    ...
    this.pos.x = clamp(this.pos.x, -this.game.city.size / 2 - 60, this.game.city.size / 2 + 150); this.pos.z = clamp(this.pos.z, -this.game.city.size / 2 - 130, this.game.city.size / 2 + 40);
...
class Ped {
  ...
  update(dt) {
    ...
    if (this.knockT > 0) { this.knockT -= dt; this.root.position.copy(this.pos); this.fig.update(dt, { state: 'knockout' }); if (this.knockT <= 0 && !this.angryT) this.flee = 6; return; } // out cold on the pavement
...
const MISSIONS = [
  ...
  { act: 1, giver: 'tony', reward: 700, title: 'HEAT', where: 'Little Havana',
    ...
    after: [['tony', "Ha! You drive like you were born running. Word's gonna travel — even across the causeway."]] },
...
class UI {
  ...
  phone(on) {
    ...
    const APPS = [['home', '🏠 Home'], ['gunshop', '🔫 Ammu-Bay'], ['motormax', '🚗 MotorMax'], ['bank', '🏦 Bay Mutual'], ['tonyDiner', "🍽 Tony's"], ['beach', '🌴 The Beach']];
