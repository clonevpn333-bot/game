...
function mat(c, o) {
  o = o || {};
  const p = { color: c, roughness: o.roughness != null ? o.roughness : 0.82, metalness: o.metalness != null ? o.metalness : 0.0, envMapIntensity: o.envMapIntensity != null ? o.envMapIntensity : 0.5 };
...
class Game {
  ...
  updateDayNight() {
    ...
    if (this.post && this.post.enabled) { this.post.compMat.uniforms.strength.value = 0.72 + night * 1.05; this.post.brightMat.uniforms.threshold.value = 1.0 - night * 0.62; this.post.compMat.uniforms.exposure.value = 1.38 - night * 0.44; }
  ...
  updateFx(dt) {
    ...
    for (const f of this.fx) {
      ...
      if (f.explode) { // fireball: expands + color-lerps white-hot → yellow → orange → ash, then fades
        ...
        // heat gradient: white-hot → plasma yellow → volcanic orange → carbon grey → fade
        if (t < 0.14) col.setHex(0xfffbe6); else if (t < 0.34) col.setHex(0xffd23f); else if (t < 0.62) col.setHex(0xff5a1f); else col.setHex(0x2b2b2b);
  ...
  explode(pos, scale) {
    ...
    if (d < 55) this.triggerSlowMo(0.18, 0.28, 0.35, 0.7); // cinematic hit-stop when the blast is near
