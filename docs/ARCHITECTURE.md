...
**LRU asset budget.** There is no generic texture/audio asset cache, because no
shipped title loads external textures or audio — every asset here is generated
procedurally at load, so a generic cache would be an unused abstraction. The
budget exists where there is something to evict: Voxel Drift's renderer holds a hard byte budget over
...
The spec's "Lighthouse PWA score ≥ 90" cannot be produced any more: Lighthouse
removed the PWA category in v12, and v13 no longer ships the installability
audits at all. `tools/pwa-check.mjs` checks the same criteria directly through
the browser's own manifest parser and a network-disabled reload, and the
performance side is covered by Lighthouse's remaining categories (100 across
performance, accessibility, best practices and SEO on this repo) plus
`tools/bench.mjs`.
